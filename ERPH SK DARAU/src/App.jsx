...
function LoginScreen({ onLogin, teachers }) {
  ...
  const handlePinSubmit = (e) => {
    ...
    if (selectedUser && pinInput === selectedUser.pin) {
      ...
      setErrorMsg("PIN Salah. Sila masukkan PIN anda yang sah.");
      setPinInput("");
    }
  };
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                {ADMIN_PROFILES.map(admin => (
                  <button key={admin.id} onClick={() => openPinModal(admin, 'admin')} className="w-full flex items-center gap-3 p-2 bg-blue-700/50 hover:bg-blue-500 rounded-lg text-left transition-colors border border-blue-500/30">
                    <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white"><User size={16} /></div>
            ...
            {filteredTeachers.map((teacher, index) => (
              <button key={teacher.id} onClick={() => openPinModal(teacher, 'teacher')} className="w-full flex items-center p-3 bg-white border border-gray-200 rounded-xl hover:border-blue-400 hover:shadow-md transition-all text-left group">
                ...
                {teacher.avatar ? (
                  ...
                  <div className="w-10 h-10 rounded-full mr-3 bg-slate-200 flex items-center justify-center text-slate-400 hidden sm:flex"><User size={20} /></div>
                )}
          ...
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handlePinSubmit} className="p-6">
              <div className="mb-6 text-center">
                ...
                <p className="text-xs text-gray-400 mt-2">Sila masukkan PIN peribadi anda.</p>
              </div>
...
function AnnouncementPanel({ user }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await setDoc(doc(db, DB_SETTINGS, 'announcement'), { text: announcement, isActive: isActive, type: type, updatedAt: new Date().toISOString() });
        ...
        <div className="bg-slate-100 rounded-3xl p-6 md:p-10 border-4 border-white shadow-2xl min-h-[400px] flex flex-col items-center">
          ...
          {isActive && announcement ? (
            <div className={`w-full ${style.bg} border-l-4 ${style.border} rounded-r-xl p-5 shadow-sm flex flex-col sm:flex-row gap-4 animate-in slide-in-from-top-4 duration-500`}>
              <div className={`bg-white p-2.5 rounded-full shadow-sm ${style.text} shrink-0 self-start`}>{style.icon}</div>
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-800 leading-relaxed font-medium whitespace-pre-wrap">{announcement}</p>
            ...
            <div className="w-full h-32 border-2 border-dashed border-slate-300 rounded-2xl flex items-center justify-center text-slate-400 text-xs font-bold uppercase tracking-widest bg-slate-50/50">Tiada Pengumuman Aktif</div>
          )}
  ...
  return (
    <div className="relative bg-white p-5 rounded-2xl border border-gray-200 hover:border-blue-400 cursor-pointer shadow-sm group hover:shadow-lg transition-all active:scale-[0.98]" onClick={onClick}>
      <div className="flex items-center gap-3 mb-4">
        <div className="w-10 h-10 rounded-full bg-slate-100 border border-gray-100 flex items-center justify-center text-slate-400 font-black text-xs">{teacher.name.charAt(0)}</div>
        ...
        <button onClick={(e) => { e.stopPropagation(); sendIndividualEmail(teacher, 'general'); }} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-full transition-all" title="Hubungi Guru"><Mail size={16} /></button>
      </div>
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-250px)]">
      ...
      <div className="lg:w-2/3 bg-white rounded-2xl shadow-sm border border-gray-200 p-6 overflow-y-auto custom-scrollbar">
        <div className="flex items-center gap-4 mb-6 border-b border-gray-50 pb-4">
          ...
          <div>
            <h3 className="font-extrabold text-lg text-gray-900 leading-tight">{teacher.name}</h3>
            <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full uppercase tracking-widest">{teacher.subject}</span>
          </div>
        </div>
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
          {Array.from({ length: TOTAL_WEEKS }, (_, i) => i + 1).map(week => {
            ...
            return (
              <button key={week} onClick={() => sub ? handleSelectWeek(week) : null}
                className={`relative h-16 rounded-xl border-2 flex flex-col items-center justify-center transition-all
                  ${sub ? (sub.status === 'graded' ? `shadow-inner ${getGradeColor(sub.score)}` : 'bg-blue-50 border-blue-200 text-blue-700') : 'bg-slate-50 border-slate-100 text-slate-300 cursor-not-allowed'}
                  ${selectedWeek === week ? 'ring-4 ring-blue-200 border-blue-500 scale-105 z-10' : ''}`}>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-blue-100 p-6 h-full flex flex-col animate-in slide-in-from-right-4 duration-300 overflow-y-auto">
            ...
            <div className={`p-4 rounded-xl border mb-6 ${submissions[selectedWeek].isLate ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`font-black text-xs ${submissions[selectedWeek].isLate ? 'text-red-800' : 'text-green-800'}`}>
                  {submissions[selectedWeek].isLate ? 'DIHANTAR LEWAT' : 'TEPAT MASA'}
                </span>
              </div>
            ...
            <a href={submissions[selectedWeek].content} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all group mb-6 shrink-0">
              ...
              <div className="overflow-hidden flex-1">
                <label className="text-[9px] font-black text-gray-400 uppercase block mb-1">Pautan RPH</label>
              ...
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 mt-auto space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {GRADE_OPTIONS.map(g => (
                      <button key={g} onClick={() => setGradeData({ ...gradeData, score: g })}
                        className={`py-2.5 px-3 rounded-xl text-xs font-extrabold border-2 transition-all ${gradeData.score === g ? `${getGradeColor(g)} border-current shadow-md scale-[1.02]` : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'}`}>
                        {g}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={saveGrade} disabled={isSaving || !gradeData.score} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-xs font-black shadow-lg shadow-blue-100 uppercase hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed">
                    {isSaving ? '...' : 'SIMPAN'}
                  </button>
                </div>
              </div>
              ...
              <div className="mt-auto space-y-3 pb-2">
                ...
                {submissions[selectedWeek].score && (
                  <div className={`p-5 rounded-2xl border-2 text-center shadow-inner mb-4 ${getGradeColor(submissions[selectedWeek].score)}`}>
                    ...
                    {submissions[selectedWeek].comment && (
...
function TeacherPortal({ user, profile }) {
  ...
  const handleDeleteSubmission = async () => {
    try { await deleteDoc(doc(db, DB_COLLECTION, `${profile.id}_w${selectedWeek}`)); }
    catch (e) { console.error("Gagal memadam", e); }
  };
        ...
        <div className="flex md:flex-col overflow-x-auto md:overflow-y-auto p-2 space-x-2 md:space-x-0 md:space-y-1 custom-scrollbar">
          {Array.from({ length: 42 }, (_, i) => i + 1).map(w => (
            <button key={w} onClick={() => setSelectedWeek(w)} className={`flex-shrink-0 md:w-full text-left px-4 py-3 rounded-xl text-xs transition-all flex flex-col ${selectedWeek === w ? 'bg-blue-600 text-white shadow-xl transform scale-[1.02]' : 'hover:bg-gray-100 text-gray-600'}`}>
              ...
              <span className={`text-[9px] mt-0.5 font-bold ${selectedWeek === w ? 'text-blue-200' : 'text-gray-400'}`}>
                {calendarData[`week_${w}_start`] ? formatDateRange(calendarData[`week_${w}_start`], calendarData[`week_${w}_end`]) : '...'}
              </span>
            </button>
          ))}
        </div>
      ...
      <div className="md:col-span-8 lg:col-span-9 bg-white border border-gray-200 rounded-3xl p-6 md:p-10 flex flex-col items-center shadow-sm relative overflow-hidden min-h-[500px] order-1 md:order-2">
        ...
        <div className="relative z-10 w-full max-w-lg text-center border-t border-gray-50 pt-8 mt-2">
          <div className="mb-8">
            ...
            <h3 className="text-blue-600 font-black text-xl md:text-2xl">
              {calendarData[`week_${selectedWeek}_start`] ? formatDateRange(calendarData[`week_${selectedWeek}_start`], calendarData[`week_${selectedWeek}_end`]) : "Tarikh Belum Ditetapkan"}
            </h3>
          ...
          {submission ? (
            <div className="space-y-6 animate-in zoom-in duration-500 w-full">
              ...
              <div className={`p-8 rounded-[2rem] border-2 flex flex-col items-center shadow-2xl ${submission.isLate ? 'border-red-100 bg-red-50/50' : 'border-green-100 bg-green-50/50'}`}>
                ...
                <h3 className={`text-2xl font-black ${submission.isLate ? 'text-red-700' : 'text-green-700'} uppercase tracking-widest`}>
                  {submission.isLate ? 'LEWAT HANTAR' : 'TELAH DIHANTAR'}
                </h3>
              ...
              <a href={submission.content} target="_blank" rel="noopener noreferrer" className="bg-white p-5 rounded-2xl border border-gray-200 shadow-md text-left flex items-center gap-4 hover:border-blue-500 transition-all group">
                ...
                <div className="flex-1 overflow-hidden">
                  <label className="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-0.5">FOLDER GOOGLE DRIVE</label>
            ...
            <form onSubmit={handleTurnIn} className="space-y-6 bg-white p-2 rounded-3xl w-full animate-in fade-in">
              <div className="bg-amber-50 border border-amber-200 p-5 rounded-2xl flex gap-4 text-xs text-amber-900 text-left items-start shadow-inner">
                ...
                <div>
                  <p className="font-black uppercase mb-1 tracking-widest">Sila Ambil Perhatian</p>
                  <p className="leading-relaxed font-medium">Sila pastikan link Google Drive anda telah ditetapkan kepada <strong>"Anyone with the link can view"</strong> sebelum menghantar.</p>
                </div>
              </div>
