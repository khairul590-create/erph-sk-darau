...
if (typeof __firebase_config !== 'undefined') {
  // MOD A: Persekitaran Paparan / Preview (Mencegah Skrin Putih & Ralat Permission)
  const config = typeof __firebase_config === 'string' ? JSON.parse(__firebase_config) : __firebase_config;
  app = initializeApp(config);
  auth = getAuth(app);
  db = getFirestore(app);
  appId = typeof __app_id !== 'undefined' ? __app_id : 'erph-sk-darau';
} else {
  // MOD B: Mod Sebenar (VS Code / Vercel / Live)
  const liveConfig = {
    // PENTING UNTUK VERCEL CIKGU:
    // Buang tanda komen (//) pada baris `apiKey: import.meta...` di bawah bila guna di VS Code.
    // Pastikan Vercel ada VITE_GOOGLE_API_KEY.
    // apiKey: import.meta.env.VITE_GOOGLE_API_KEY, 
    
    apiKey: "import.meta.env.VITE_GOOGLE_API_KEY,", // Padam baris ini dalam VS Code nanti
...
// Helper Path (Berfungsi ganda: untuk Preview dan Live DB Sebenar)
const getAppCollection = (name) => {
    if (typeof __firebase_config !== 'undefined') return collection(db, 'artifacts', appId, 'public', 'data', name);
    return collection(db, name); // Root collection dalam Firebase sebenar
};

const getAppDoc = (colName, documentId) => {
    if (typeof __firebase_config !== 'undefined') return doc(db, 'artifacts', appId, 'public', 'data', colName, documentId);
    return doc(db, colName, documentId); // Root doc dalam Firebase sebenar
};
...
function AdminDashboard({ user, teachers, currentProfile }) {
  ...
  useEffect(() => {
    ...
    const fetchAnnouncement = async () => {
      try {
        const annDoc = await getDoc(getAppDoc(DB_SETTINGS, 'announcement'));
...
function AnnouncementPanel({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const calDoc = await getDoc(getAppDoc(DB_SETTINGS, 'school_calendar'));
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await setDoc(getAppDoc(DB_SETTINGS, 'announcement'), {
        text: announcement,
        isActive: isActive,
        type: type,
...
function CalendarSettingsPanel({ user }) {
  ...
  useEffect(() => {
    ...
    const fetchCalendar = async () => {
      try {
        const docSnap = await getDoc(getAppDoc(DB_SETTINGS, 'school_calendar'));
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await setDoc(getAppDoc(DB_SETTINGS, 'school_calendar'), calendarData);
...
function EmailAutomationPanel({ teachers, user }) {
  ...
  const scanForLateSubmissions = async () => {
    ...
    try {
      const q = query(getAppCollection(DB_COLLECTION), where('week', '==', parseInt(targetWeek)));
...
function TeacherCard({ teacher, onClick, user }) {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(getAppCollection(DB_COLLECTION), where('teacherId', '==', teacher.id));
...
function AdminGradingView({ user, teacher, currentProfile }) {
  ...
  const saveGrade = async () => {
    ...
    setIsSaving(true);
    try {
      await updateDoc(getAppDoc(DB_COLLECTION, submissions[selectedWeek].id), {
  ...
  const handleAdminDelete = async () => {
      try {
          await deleteDoc(getAppDoc(DB_COLLECTION, submissions[selectedWeek].id));
          setConfirmAdminDelete(false);
...
function TeacherPortal({ user, profile }) {
  ...
  useEffect(() => {
    if(!user) return;
    getDoc(getAppDoc(DB_SETTINGS, 'school_calendar')).then(s => s.exists() && setCalendarData(s.data()));
    const unsubAnn = onSnapshot(getAppDoc(DB_SETTINGS, 'announcement'), (snap) => {
  ...
  useEffect(() => {
    if (!user) return;
    const unsub = onSnapshot(getAppDoc(DB_COLLECTION, `${profile.id}_w${selectedWeek}`), (docSnap) => {
  ...
  const handleTurnIn = async (e) => {
    ...
    try {
        await setDoc(getAppDoc(DB_COLLECTION, `${profile.id}_w${selectedWeek}`), {
  ...
  const handleDeleteSubmission = async () => {
    try { await deleteDoc(getAppDoc(DB_COLLECTION, `${profile.id}_w${selectedWeek}`)); } catch (e) { console.error("Gagal memadam", e); }
  };
