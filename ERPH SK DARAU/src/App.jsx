...
// --- FIREBASE CONFIGURATION (SECURE MODE) ---
// Dalam persekitaran ini, config diambil secara automatik dari sistem pelayan
let firebaseConfig = {};
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'erph-sk-darau';
...
function TeacherPortal({ user, profile }) {
  ...
  useEffect(() => {
    if (!user) return;
    const unsub = onSnapshot(getPublicDoc(DB_COLLECTION, `${profile.id}_w${selectedWeek}`), (doc) => {
        setSubmission(doc.exists() ? doc.data() : null);
